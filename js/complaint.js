...
var vm = new Vue({
        ...
        submitComplain() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    images: imgs.join(','),
                    role_id:this.sectionId,
                    admin_id:this.leaderId
                },
                dataType: 'json',
    ...
    mounted() {   
        initSectionPicker('choose-section','选择部门');
    }
});
...
function initSectionPicker(elemId,title) {
    ...
    sectionPicker.setData(data);
    var eventBtn = document.getElementById(elemId);
    eventBtn.addEventListener('tap', function (event) {
        sectionPicker.show(function (items) {
            if(vm.sectionId != items[0].value) {
              vm.sectionId = items[0].value;
              vm.sectionName = items[0].text;
              initLeaderPicker('choose-section-leader','选择负责人',items[0].value);  
            }
        });
    }, false);
}

function initLeaderPicker(elemId,title,sectionId) {
    vm.leaderId = '';
    vm.leaderName = '';
    if(leaderPicker == '') {
        leaderPicker = new mui.PopPicker({
            title: title
        });
    }
    var data = getLeaderListBySectionId(sectionId).map(function (item) {
        return {
            value: item.id,
            text: item.username
        }
    });
    leaderPicker.setData(data);
    if(leaderPicker == '') {
        return;
    }
    var eventBtn = document.getElementById(elemId);
    eventBtn.addEventListener('tap', function (event) {
        leaderPicker.show(function (items) {
            vm.leaderId = items[0].value;
            vm.leaderName = items[0].text;
        });
    }, false);
}
/**
 ...
 * 获得部门对应的负责人
 */
function getLeaderListBySectionId(sectionId) {
    var res = '';
    $.ajax({
        url:`${rootUrl}/index/api/getComplainRoleAdmin`,
        type:'post',
        dataType:'json',
        async:false,
        data:{
